...
export class GameRoomComponent implements OnInit, OnDestroy {
  ...
  copyLinkToClipboard(): void {
    ...
    navigator.clipboard.writeText(url).then(() => {
      ...
      this.toastr.error('An error occurred while copying the link to clipboard.', 'Error');
    });
  }
  ...
  toggleReady() {
    ...
    this.lobbyService.setReadyStatus(this.roomId, newReadyStatus, this.userProfile.id).subscribe(
      ...
      (error) => {
        this.toastr.error('An error occurred while setting the ready status.', 'Error');
        this.router.navigate(['/games/online-pvp/rooms']);
      }
    );
  }
  ...
  ngOnDestroy() {
    ...
    loadFriends(): void {
      this.friendService.getFriends().subscribe(
        ...
        (error) => {
          this.toastr.error('An error occurred while fetching friends list.', 'Error');
        }
      );
    }
